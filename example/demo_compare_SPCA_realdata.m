...
function demo_compare_SPCA_realdata()
    ...
    save_root = strcat('../test/spca-real/');
    ...
    fileName = {dirOutput.name}; % The names of .mat files
    ...
    rng(1000); test_num = 1;
    table_str = '';
    % prob = 1:length(fileName);
    prob = 1;

    %% cycle
    for i = prob % for all datasets
        % Load the data and construct the problem matrix
        base = fileName{i};
        ...
        for id_r = 1:size(r_set, 1) % r  number of column
            ...
            for id_mu = 1:size(mu_set, 1) % mu  sparse parameter
                ...
                basename = strrep(base, '.mat', ''); % remove the extension
                ...
                table_str = [table_str sprintf('& %.3e & %.2f', ...
                            out_mialm.obj, out_mialm.time)];
